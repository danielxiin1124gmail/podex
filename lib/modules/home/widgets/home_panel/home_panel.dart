...
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_favorites.dart';

import 'package:pokedex/modules/home/widgets/home_panel/panels/pokemon_type_filter.dart';
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_type_filter.dart';
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_style_filter.dart';
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_country_filter.dart';
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_rightOrLeft_filter.dart';
import 'package:pokedex/modules/home/widgets/home_panel/panels/tennisPlayer_backhandStyle_filter.dart';
...
class HomePanelWidget extends StatelessWidget {
  ...
  final TennisItemStore tennisItemStore;
            ...
            return Stack(
              children: [
                ...
                if (homePageStore.panelType ==
                  ...
                  TennisPlayerStyleFilter(
                    homePageStore: homePageStore,
                    scrollController: scrollController,
                  ),
                if (homePageStore.panelType ==
                    PanelType.FILTER_TENNIS_PLAYER_COUNTRY)
                  TennisPlayerCountryFilter(
                    homePageStore: homePageStore,
                    scrollController: scrollController,
                  ),
                if (homePageStore.panelType ==
                    PanelType.FILTER_TENNIS_PLAYER_RIGHT_OR_LEFT)
                  TennisPlayerRightOrLeftFilter(
                    homePageStore: homePageStore,
                    scrollController: scrollController,
                  ),
                if (homePageStore.panelType ==
                    PanelType.FILTER_TENNIS_PLAYER_BACKHAND_STYLE)
                  TennisPlayerBackhandStyleFilter(
                    homePageStore: homePageStore,
                    scrollController: scrollController,
                  ),
                ...
                if (homePageStore.panelType ==
                  ...
                  TextFilterWidget(
                    hintText: "Ex: Federer or 001", // 搜索栏位初始 hint。
                    text: tennisPlayerStore
                        .tennisPlayerFilter.tennisPlayerNameNumberFilter,
                    homePageStore: homePageStore,
                    onChanged: (value) {
                      tennisPlayerStore.setNameNumberFilter(value);
                    },
                    onClose: () {
                      panelController.close();
                      tennisPlayerStore.clearNameNumberFilter();
                      // clearNameNumberFilter 很重要，一旦关闭文字搜索，才能显示回全部怪兽。
                    },
                  ),
                //-----------------------------------------------------//
                ...
                // 下面是 tennisItem的filter设定，在此把输入文字到素电脑，文字=filter。
                if (homePageStore.panelType == PanelType.FILTER_TENNIS_ITEMS)
                  TextFilterWidget(
                    hintText: "Ex: Tennis Ball",
                    text: tennisItemStore.filter,
                    // filter 是在 item_store.g.dart里面。
                    // 上面两个TextFilterWidget中的这行逻辑不太一样。当前不明白。
                    homePageStore: homePageStore,
                    onChanged: (value) {
                      tennisItemStore.setFilter(value);
                    },
                    onClose: () {
                      panelController.close();
                      tennisItemStore.clearFilter();
                      // clearFilter 很重要，一旦关闭文字搜索，才能显示回全部物品。
                    },
                  ),
                //-----------------------------------------------------//
