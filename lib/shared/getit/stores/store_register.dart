...
class StoreRegister extends IGetItRegister {
  ...
  void register() {
    ...
    if (!GetIt.I.isRegistered<PokemonStore>()) {
      print(
          '<store_registor.dart> 准备执行 getIt.registerSingleton<PokemonStore>(PokemonStore()');
      getIt.registerSingleton<PokemonStore>(PokemonStore());
    }

    if (!GetIt.I.isRegistered<ItemStore>()) {
      print(
          '<store_registor.dart> 准备执行 getIt.registerSingleton<ItemStore>(ItemStore()');
      getIt.registerSingleton<ItemStore>(ItemStore());
    }

    if (!GetIt.I.isRegistered<HomePageStore>()) {
      print(
          '<store_registor.dart> 准备执行 getIt.registerSingleton<HomePageStore>(HomePageStore()');
